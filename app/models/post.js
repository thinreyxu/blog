module.exports = (dbd, marked, time) => {
  const { DBC, ObjectID } = dbd
  const { makeMd, makeSummary } = marked
  const { makeTime } = time
  ...
  class Post {
    ...
    static async update ({ id, title, tags, content }) {
      ...
      try {
        let _id = new ObjectID(id)
        db = await DBC.connect()
        let summary = await makeSummary(content)
        let data = { title, tags, content, summary }
        await db.collection(COLLECTION_NAME)
...
module.exports['@require'] = [ 'modelsOld/db', 'lib/marked', 'lib/time' ]
